...
export const DetailScreen: React.FC = () => {
    ...
    const conditionalContainerStyle: ViewStyle = useMemo(() => ({ backgroundColor: launch.upcoming ? 'rgba(0,0,0)' : launch.success ? 'rgba(38,194,129)' : 'rgba(255,0,0)' }), [launch]);


    return (
        <ScrollView
            contentContainerStyle={[styles.container, conditionalContainerStyle]}
            style={conditionalContainerStyle}
            refreshControl={<RefreshControl refreshing={isLoading}
                onRefresh={fetchRocket} />}
        >
            ...
            {!!launch.failures.length && <Text style={styles.subtitle}>{`Failure Reasons: ${formatLaunchFailures(launch)}`}</Text>}
            ...
            />
            {!!rocket && <View style={styles.rocketContainer}>
